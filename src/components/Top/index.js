...
class Top extends Component {
  ...
  getApiCall = async () => {
    ...
    const url = `https://api.themoviedb.org/3/movie/top_rated?api_key=b9eacb52c8666ad78aa34501c81fbe03&language=en-US&page=${currentPage}`
    ...
    this.setState({topArray: updatedData, totalPages: data.total_pages})
  }
  ...
  render() {
    const {topArray, movies, currentPage, currentPageMovie} = this.state
    const {userInput} = this.state
    return (
      <>
        <div className="header">
          <h1 className="titleName">Top Rated</h1>
